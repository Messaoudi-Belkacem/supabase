            ...
            <ul className="flex flex-col gap-4">
              {column.links.map(({ icon: Icon, ...link }: LinkProps) => (
                <li key={link.text}>
                  <Link
                    href={link.url!}
                    className="flex group items-center gap-2 text-foreground-light text-sm hover:text-foreground focus-visible:text-foreground focus-visible:ring-2 focus-visible:outline-hidden focus-visible:rounded focus-visible:ring-foreground-lighter"
                  >
          ...
          <ul className="flex flex-col gap-5">
            {latestBlogPosts?.slice(0, 2).map((post: any) => (
              <li key={post.title}>
                <Link
                  href={post.url}
                  className="group flex flex-col focus-visible:ring-2 focus-visible:outline-hidden focus-visible:ring-foreground-lighter focus-visible:ring-offset-4 focus-visible:ring-offset-background-alternative focus-visible:rounded"
                >
